...
object ModelGens {
  ...
  private val playerIdGen: Gen[Player.Id]        = Gen.const(Player.Id(UUID.V4.random))
  implicit val playerIdArb: Arbitrary[Player.Id] = Arbitrary(playerIdGen)

  private val cardClassIdGen: Gen[CardClass.Id]        = Gen.const(CardClass.Id(UUID.V4.random))
