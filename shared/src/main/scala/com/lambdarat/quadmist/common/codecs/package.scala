...
package object codecs {
  ...
  /* ---- DECODERS ---- */
  implicit val cardClassIdDecoder   = Decoder[UUID].map(CardClass.Id.apply)
  implicit val cardClassNameDecoder = Decoder.decodeString.map(CardClass.Name.apply)

  implicit val playerIdDecoder = Decoder[UUID].map(Player.Id.apply)

  implicit val cardIdDecoder            = Decoder[UUID].map(Card.Id.apply)
